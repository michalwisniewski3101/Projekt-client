<template>
    ...
    filteredTasks() {
      return this.tasks.filter(task => {
        const values = Object.values(task).map(value => value ? value.toString().toLowerCase() : '');
        const search1Match = this.search1 === '' || values.some(value => value.includes(this.search1.toLowerCase()));

        const search2Match = this.search2 === '' || task.server_name.toLowerCase().includes(this.search2.toLowerCase());
